...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [vesselsRes, timelineRes, conflictsRes, kpisRes] = await Promise.all([
        axios.get(`${API}/vessels`),
        axios.get(`${API}/berths/timeline`),
                ...
                {vessels.map(vessel => (
                  <tr key={vessel.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <div>
                        {vessel.eta?.estimado && (
                          <div>ETA: {new Date(vessel.eta.estimado).toLocaleString('pt-BR', { month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</div>
                        )}
                        {vessel.ata && (
